...
class BorderLayout @JvmOverloads constructor(
    ...
    init {
        attrs?.let {
            val ta = context.obtainStyledAttributes(it, R.styleable.BorderLayout)
            paintBorder.strokeWidth = ta.getDimensionPixelSize(R.styleable.BorderLayout_bl_border_size,0).toFloat()
            paintBorder.color = ta.getColor(R.styleable.BorderLayout_bl_border_color,
                Color.TRANSPARENT)
            borderCorner = ta.getDimensionPixelSize(R.styleable.BorderLayout_bl_border_corner,0).toFloat()
