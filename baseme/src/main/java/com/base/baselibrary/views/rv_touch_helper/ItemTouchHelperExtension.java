...
public class ItemTouchHelperExtension extends RecyclerView.ItemDecoration
    ...
    private final RecyclerView.OnItemTouchListener mOnItemTouchListener
        ...
        public boolean onInterceptTouchEvent(RecyclerView recyclerView, MotionEvent event) {
            ...
            loge("intercept: x:" + event.getX() + ",y:" + event.getY() + ", " + event);
            final int action = event.getActionMasked();
            if (action == MotionEvent.ACTION_DOWN) {
                loge("InterceptTouch: Down");
        ...
        public void onTouchEvent(RecyclerView recyclerView, MotionEvent event) {
            mGestureDetector.onTouchEvent(event);
            loge("on touch: x:" + mInitialTouchX + ",y:" + mInitialTouchY + ", :" + event);
            if (mVelocityTracker != null) {
                mVelocityTracker.addMovement(event);
            }
    ...
    public void startDrag(ViewHolder viewHolder) {
        if (!mCallback.hasDragFlag(mRecyclerView, viewHolder)) {
            loge("Start drag has been called but swiping is not enabled");
            return;
        }
    ...
    public void startSwipe(ViewHolder viewHolder) {
        if (!mCallback.hasSwipeFlag(mRecyclerView, viewHolder)) {
            loge("Start swipe has been called but dragging is not enabled");
            return;
        }
        if (viewHolder.itemView.getParent() != mRecyclerView) {
